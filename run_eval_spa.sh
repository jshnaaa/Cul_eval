...
# CultureSPA模型评估运行脚本
# 使用方法: bash run_eval_spa.sh <DATA_ID>
# 示例: bash run_eval_spa.sh 2
...
if [ $# -ne 1 ]; then
    ...
    echo "使用方法: bash run_eval_spa.sh <DATA_ID>"
    echo "示例: bash run_eval_spa.sh 2"
    exit 1
fi
...
if [ ! -f "./eval_spa.py" ]; then
    echo "错误：评估脚本不存在: ./eval_spa.py"
    exit 1
fi
